...
/**
 * 在线模式，应对HTML5的offline功能(application cache)
 * 应用的src文件夹（所有源码文件）放在在服务器上
 * app.xml和图片放在在本地
 *
 */
public class XOnlineMode extends XAppRunningMode implements XAppCheckListener {
    ...
    public void clearOfflineAppCache(String appUrl) {
        File file = new File(mDataBasePath);
        if( !file.exists() ) {
            XLog.e(CLASS_NAME, "clearAppCache: ApplicationCache.db is not exists!");
            return;
        }
        SQLiteDatabase database = SQLiteDatabase.openOrCreateDatabase(mDataBasePath, null);
        try {
            ...
            database.delete("CacheAllowsAllNetworkRequests", "cache = \'" + cache + "\'", null);
            ...
            database.delete("CacheResourceData", "id in ( select resource from " +
                            "CacheEntries where cache = \'" + cache + "\')", null);
            //删除CacheEntries表中当前应用的对应项
            database.delete("CacheEntries", "cache = \'" + cache + "\'", null);
            //删除CacheGroups表中当前应用的对应项
            database.delete("CacheGroups", "newestCache = \'" + cache + "\'", null);
            //删除CacheWhiltelistURLs表中当前应用的对应项
            database.delete("CacheWhitelistURLs", "cache = \'" + cache + "\'", null);
        } catch (SQLiteException e) {
            XLog.d(CLASS_NAME, "SQLite operate exception!");
            e.printStackTrace();
        } finally {
    ...
    class LocalStorageClear implements XFileVisitor {
        ...
        public void visit(String filePath) {
            ...
            if (file.getName().equals(mFileName)) {
                ...
                if (!file.exists()) {
                    XLog.d(CLASS_NAME,"clearAppLocalStorage: " + file.getAbsolutePath()
                                    + " is not exists!");
                    return;
                }
     ...
     * @return    在线应用所在localstorage数据库
     *            例如:"http_com.polyvi.net_8012.localstorage"
     */
