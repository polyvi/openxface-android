...
public class XConfiguration {
    ...
    /**
     * 获取系统允许执行的扩展列表，每一项代表一个扩展的名称
     */
    public HashMap<String, XExtensionEntry> getSysExtensions() {
        return (null == mSysConfigInfo) ? null : mSysConfigInfo
                .getSysExtensions();
    }

    /**
     * 从config.xml配置文件中读取要加载的扩展
     */
    public HashMap<String, XExtensionEntry> readLoadingExtensions(
            XExtensionContext extensionContext) {
        if (null != mExtensionsCache) {
            return mExtensionsCache;
        }
        mExtensionsCache = getSysExtensions();
        return mExtensionsCache;
    }
