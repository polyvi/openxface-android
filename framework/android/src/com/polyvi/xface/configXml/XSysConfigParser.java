...
public class XSysConfigParser {
     ...
     * @return 扩展集合
     */
    private HashMap<String, XExtensionEntry> parseExtensionTag() {
        NodeList nodes = mDoc.getElementsByTagName(TAG_EXTENSION);
        int len = (null == nodes ? 0 : nodes.getLength());
        HashMap<String, XExtensionEntry> extensions = new HashMap<String, XExtensionEntry>(len);
        for (int i = 0; i < len; i++) {
            ...
            if(null != textChild) {
                ...
                XExtensionEntry entry = new XExtensionEntry(name, className);
